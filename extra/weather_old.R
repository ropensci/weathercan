...
weather <- function(station_ids,
  ...
  for(s in station_ids) {
    ...
    stn <- envirocan::stations %>%
      ...
      dplyr::filter(timeframe == timeframe) %>%
    ...
    if(is.null(start)) s.start <- stn$start else s.start <- start
    if(is.null(end)) s.end <- stn$end else s.end <- end
    ...
    date_range <- seq(floor_date(s.start, unit = "month"),
                      ...
                      by = ifelse(timeframe %in% c("hour", "day"), "month", "year"))
