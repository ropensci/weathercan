...
test_that("approx_na_rm (time) without NAs", {
  ...
  ## Need same TZ
  expect_error(
    approx_na_rm(x = k$time, y = k$temp, xout = f$time, na_gap = 1),
  ...
  for (i in c(0, 2, lubridate::hours(2), lubridate::days(14))) {
    expect_silent(
      a <- approx_na_rm(x = k$time, y = k$temp, xout = f$time, na_gap = i)
    )
...
test_that("approx_na_rm (time) without NAs pads NAs at start/end", {
  ...
  expect_true(all(is.na(a$y[
    a$x < as.POSIXct("2016-03-08 12:00:00", tz = "Etc/GMT+8")
  ])))
  expect_true(all(is.na(a$y[
...
test_that("weather_interp (hour) fails with incorrect data types", {
  ...
  ## Expect failure
  expect_error(
    weather_interp(f, dplyr::mutate(k, time = as.numeric(time))),
    regexp = paste0(
      "'data' and 'weather' must be data frames with ",
      "columns 'time' in POSIXct format or 'date' in ",
      "Date format."
    )
  )
  expect_error(
    weather_interp(dplyr::mutate(f, time = as.numeric(time)), k),
    regexp = paste0(
      "'data' and 'weather' must be data frames with ",
      "columns 'time' in POSIXct format or 'date' in ",
      "Date format."
...
test_that("weather_interp (hour) interpolates 'all'", {
  ...
  expect_named(
    ...
    c(
      names(f),
      c("temp", "temp_dew", "rel_hum", "wind_spd", "visib", "pressure")
    )
  )
  expect_equal(a[, seq_len(ncol(f))], f)
})
...
test_that("weather_interp (hour) fails on character columns", {
  ...
  expect_message(
    weather_interp(f, k, cols = c("temp", "rel_hum")),
    "Some columns \\(temp\\)"
  )
})
...
test_that("weather_interp (day) fails with incorrect data types", {
  ...
  expect_error(
    weather_interp(
      f,
      dplyr::mutate(k, date = as.numeric(date)),
      interval = "day"
    ),
    regexp = paste0(
      "'data' and 'weather' must be data frames ",
  ...
  expect_error(
    weather_interp(
      dplyr::mutate(f, date = as.numeric(date)),
      k,
      interval = "day"
    ),
    regexp = paste0(
  ...
  for (m in c(
    ...
    if (any(is.na(k[, m]))) {
      expect_message(expect_error(
        a <- weather_interp(f, k, cols = m, interval = "day"),
        NA
      ))
    } else {
...
test_that("weather_interp (day) skips character columns", {
  ...
  expect_error(
    expect_message(
      weather_interp(f, k, cols = "max_temp", interval = "day"),
      paste0(
        "Some columns \\(max\\_temp\\) are not ",
  ...
  expect_message(
    weather_interp(f, k, cols = c("max_temp", "mean_temp"), interval = "day"),
